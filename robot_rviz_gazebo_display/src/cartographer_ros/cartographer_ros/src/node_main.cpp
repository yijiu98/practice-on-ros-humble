...
void Run() {
  ...
  NodeOptions node_options;//节点
  TrajectoryOptions trajectory_options;//轨迹
...
int main(int argc, char** argv) {
  ...
  google::AllowCommandLineReparsing();//允许对命令行参数进行重新解析
  google::InitGoogleLogging(argv[0]);// note: 初始化glog库
  // 使用gflags进行参数的初始化. 其中第三个参数为remove_flag
  // 如果为true, gflags会移除parse过的参数, 否则gflags就会保留这些参数, 但可能会对参数顺序进行调整.
  // 这里获取到launch中节点启动时的args参数
   ...
   * 它会注册一个日志接收器，用于拦截 glog 输出的日志信息
   */
  cartographer_ros::ScopedRosLogSink ros_log_sink;
  cartographer_ros::Run();
  ::rclcpp::shutdown();// 结束ROS相关的线程, 网络等
}
